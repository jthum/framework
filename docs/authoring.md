# Svelte authoring interfaces

Authoring owns the editor interfaces, draft interactions and reusable controls. A host owns navigation, authorization, persistence, execution and data previews. There are no SvelteKit imports, global sessions or SQL in these components.

## Code map

- Public exports: [`src/svelte/authoring/index.ts`](../src/svelte/authoring/index.ts)
- Working models and host-facing data contracts: [`src/svelte/authoring/data.ts`](../src/svelte/authoring/data.ts)
- Canonical adapter barrel: [`src/svelte/authoring/adapters.ts`](../src/svelte/authoring/adapters.ts)
- Context projection: [`src/svelte/authoring/context-adapter.ts`](../src/svelte/authoring/context-adapter.ts)
- Authoring tests: [`src/svelte/authoring`](../src/svelte/authoring)
...
Authoring does not offer a generic runtime-Action editor. Runtime Action implementations are
...
`CollectionDraft`, `FieldDraft`, `ViewDraft`, `FormDraft` and `RuleDraft` are UI working models, not a second accepted Spec version. They use editable keys for picker choices. Canonical adapters resolve those keys to portable definition identities when persisting. Arbitrary `meta` survives draft edits unchanged. Authoring does not perform schema migrations or authorize sharing based on presentation hints.
...
references, stable step identities, nested control flow, and optional run-as bindings. Authoring keeps
its key-oriented working vocabulary private and automatically assigns missing step identities.
Execution remains a Kernel responsibility rather than being copied into Authoring.

Friendly Rule-authoring steps translate to executable canonical inputs: “Update current record”
...
bindings while Authoring continues to show their editable keys. “Query records” becomes `records.list` with an identity-based
...
`editorContextFromSpec` projects canonical Collections, Views, Forms and Rules into one authoring
...
Authoring consumers can style the semantic tokens and compose host preview snippets. Runtime-specific compatibility diagnostics and registries must come from the host, never an assumed browser runtime.

All five full editors accept `class` for outer layout adjustments. Their controls use the shared chrome Card and semantic tokens, so the implementing app retains its theme rather than adopting a hard-coded component theme. The shadcn-Svelte extraction keeps the existing chrome/header/body/footer composition intact.

## Related operations interface

`ExecutionList`, `RuleLauncher`, `ExecutionInspector`, and `ActorRequestCard` are reusable Svelte
components exported from `@jthum/framework/svelte/operations`. They use chrome Cards and inherit the host theme.
...
the initiating Actor; the Kernel requires a User assignee. No actor directory is invented by Authoring.
...
`@jthum/framework/svelte/authoring/rule-editor`. Import working models, authoring actions and
metadata from `@jthum/framework/svelte/authoring/data`. Import operational components from
`@jthum/framework/svelte/operations` or their granular subpaths. The aggregate
`@jthum/framework/svelte/authoring` barrel remains a convenience for tools and environments where
eagerly loading all authoring surfaces is intentional. Product shells should prefer granular paths
