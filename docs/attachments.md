...
`rights` defaults to `["read"]`; `allowReshare` defaults to `false`. Creation records `createdBy` and `createdAt`. Revocation records `revokedBy` and `revokedAt`, is idempotent, and cannot be undone. Either endpoint may revoke subject to authorization. Revoked bindings remain in `listAttachments` for provenance; the semantic key may be explicitly bound to a new Attachment afterward.
