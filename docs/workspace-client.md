...
`AgentClient` is the smaller Agent-only contract implemented by `WorkspaceClient`. Its bound Actor
must have `kind: "agent"`. `listAgentTools` exposes the current projected capabilities and
