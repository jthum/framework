...
/** Convert one canonical View into the authoring interface's key-oriented working model. */
export function viewDraftFromDefinition(
  ...
  return {
    ...
    ...(isAuthoringManaged(view.meta) ? { implicit: true } : {}),
    source: view.source,
...
/** Convert an authoring draft to a canonical View while retaining identity and opaque metadata. */
